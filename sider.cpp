...
static int sider_sprite(lua_State *L) {
    ...
    //DX11.Context->UpdateSubresource(pConstantBuffer, 0, NULL, &constants, 0, 0);
    DX11.Context->PSSetConstantBuffers( 0, 1, &pConstantBuffer );

    D3D11_VIEWPORT vp;
    vp.MinDepth = 0.0f;
    vp.MaxDepth = 1.0f;
    vp.TopLeftX = x; //rc.right - rc.left - _overlay_image.width - _overlay_image.hmargin;
    vp.TopLeftY = y; //top + _overlay_image.vmargin;
    vp.Width = width;
    vp.Height = height;
...
void module_on_frame(module_t *m)
{
    if (m->evt_display_frame != 0) {
        ...
        // push params
        lua_pushvalue(L, 1); // ctx
        lua_pushnumber(L, DX11.Width);
        lua_pushnumber(L, DX11.Height);
        if (lua_pcall(L, 3, 0, 0) != LUA_OK) {
            const char *err = luaL_checkstring(L, -1);
...
static void push_env_table(lua_State *L, module_t *m)
{
    ...
    */
    ...
    lua_pushcclosure(L, sider_sprite, 0);
    lua_settable(L, -3);
