...
function m.init(ctx)
    if ffi == nil then
        error("This module requires ffi library to be enabled. Set luajit.ext.enabled = 1 in sider.ini")
    end

    -- step 1: allocate memory. See flag definitions here:
    -- https://learn.microsoft.com/en-us/windows/win32/api/memoryapi/nf-memoryapi-virtualalloc
    local allocationFlags = 0x1000 + 0x2000
    local protection_execute_readwrite = 0x40
    local addr = ffi.C.VirtualAlloc(nil, 512, allocationFlags, protection_execute_readwrite)
    if not addr then
